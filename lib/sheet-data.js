...
const SHEET_CSV_URLS = [
  ...
  `https://docs.google.com/spreadsheets/d/e/2PACX-1vQ0HZRcKYCHl-MwPP0TWvJl2_BvCwYpRqA9vQvVOiVx76r1ypSLxsfCqNXgpCV_mRoWUh2Iesnf9bje/pub?gid=1366881810&single=true&output=csv`,
  `https://docs.google.com/spreadsheets/d/e/2PACX-1vQ0HZRcKYCHl-MwPP0TWvJl2_BvCwYpRqA9vQvVOiVx76r1ypSLxsfCqNXgpCV_mRoWUh2Iesnf9bje/pub?gid=1334662304&single=true&output=csv`,
  `https://docs.google.com/spreadsheets/d/e/2PACX-1vQ0HZRcKYCHl-MwPP0TWvJl2_BvCwYpRqA9vQvVOiVx76r1ypSLxsfCqNXgpCV_mRoWUh2Iesnf9bje/pub?gid=2044426414&single=true&output=csv`,
  `https://docs.google.com/spreadsheets/d/e/2PACX-1vQ0HZRcKYCHl-MwPP0TWvJl2_BvCwYpRqA9vQvVOiVx76r1ypSLxsfCqNXgpCV_mRoWUh2Iesnf9bje/pub?gid=1260184946&single=true&output=csv`,
];

// Documento aparte con los árbitros de cada partido (nombre, amarillas,
// rojas, y goleadores cuando lo rellenes). Un solo documento, no hace
// falta separar por pestañas porque no está dividido por jornadas.
const ARBITROS_CSV_URL = `https://docs.google.com/spreadsheets/d/e/2PACX-1vRIwjRN70I4PtYFvBrFyoedc6v_ZxzB1LBk-RkBO64t0VqVSeSvsEi-VF9YysjS8dTVyf0_qhxvkJ2v/pub?output=csv`;
...
async function getArbitrosRows() {
  return fetchOneSheet(ARBITROS_CSV_URL);
}

export function matchId(row) {
...
async function ajustarPorRival(base, rivalTeamName, arbitroFactor = null) {
  if (!base) {
  ...
  STAT_KEYS.forEach(({ key }) => {
    ...
    let multiplicador = 1;
    if (rivalVal && ligaVal) {
      multiplicador = inversa ? ligaVal / rivalVal : rivalVal / ligaVal;
    }

    // El árbitro solo afecta a las faltas (cometidas y recibidas), no a
    // remates, entradas o paradas.
    const esFaltas = key === "faltasCometidas" || key === "faltasRecibidas";
    if (esFaltas && arbitroFactor) {
      multiplicador *= arbitroFactor;
    }
...
// Cuánto más (o menos) pita este árbitro en amarillas, comparado con la
// media de todos los árbitros. 1 = normal, >1 = pita más, <1 = pita menos.
async function getArbitroFactor(arbitroName) {
  if (!arbitroName) return null;
  const arbitro = await getArbitroSeasonAverage(arbitroName);
  const liga = await getLigaArbitrosAverage();
  if (!arbitro || !liga || !liga.amarillas) return null;
  return clamp(arbitro.amarillas / liga.amarillas, AJUSTE_MIN, AJUSTE_MAX);
}

export async function getContextualPrediction(playerName, rivalTeamName, arbitroName) {
  const base = await getPlayerSeasonAverage(playerName);
  if (!base || !base.muestraSuficiente) {
    return { base, ajustado: null, sinAjuste: true };
  }
  const arbitroFactor = await getArbitroFactor(arbitroName);
  return ajustarPorRival(base, rivalTeamName, arbitroFactor);
}

export async function getTeamContextualPrediction(teamName, rivalTeamName, arbitroName) {
...
export async function getAllPlayerNames() {
  ...
  rows.forEach((row) => {
    if (row.JUGADOR) nombres.add(row.JUGADOR);
  });
  return Array.from(nombres).sort((a, b) => a.localeCompare(b, `es`));
}

export async function getPlayerMatchHistory(playerName) {
  const rows = await getAllRows();
  const playerRows = rows.filter((row) => row.JUGADOR === playerName);

  return playerRows
    .map((row) => {
      const equipoLocal = row["EQUIPO LOCAL"];
      const equipoVisitante = row["EQUIPO VISITANTE"];
      const esLocal = row.EQUIPO === equipoLocal;
      const rival = esLocal ? equipoVisitante : equipoLocal;
      return {
        id: matchId(row),
        fecha: row.FECHA,
        jornada: row.JORNADA,
...
export async function getArbitroDePartido(id) {
  const rows = await getArbitrosRows();
  const row = rows.find((r) => matchId(r) === id);
  if (!row) return null;
  return {
    nombre: row.ARBITRO,
    amarillas: toNumber(row.AMARILLAS),
    rojas: toNumber(row.ROJAS),
  };
}

export async function getArbitroSeasonAverage(arbitroName) {
  const rows = await getArbitrosRows();
  const suyos = rows.filter((r) => r.ARBITRO === arbitroName);
  if (suyos.length === 0) return null;

  const numPartidos = suyos.length;
  const amarillas = suyos.reduce((acc, r) => acc + (toNumber(r.AMARILLAS) || 0), 0) / numPartidos;
  const rojas = suyos.reduce((acc, r) => acc + (toNumber(r.ROJAS) || 0), 0) / numPartidos;

  return { amarillas, rojas, numPartidos };
}

export async function getLigaArbitrosAverage() {
  const rows = await getArbitrosRows();
  if (rows.length === 0) return null;

  const numPartidos = rows.length;
  const amarillas = rows.reduce((acc, r) => acc + (toNumber(r.AMARILLAS) || 0), 0) / numPartidos;
  const rojas = rows.reduce((acc, r) => acc + (toNumber(r.ROJAS) || 0), 0) / numPartidos;
