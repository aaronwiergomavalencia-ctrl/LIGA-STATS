...
// Calcula la media por 90' de un conjunto de filas (de un jugador o de un equipo entero).
...
// Media por 90' de TODOS los jugadores de un equipo (para caracterizar su estilo de juego).
...
// Media de TODA la liga (todos los equipos), como punto de referencia neutral.
export async function getLeagueAverage() {
  const rows = await getAllRows();
  return mediaPor90(rows);
}

// Qué estadística del RIVAL usamos como "contexto" para ajustar cada stat propia,
// y si la relación es directa (rival alto → mi stat sube) o inversa (rival alto → mi stat baja).
const CONTEXTO = {
  remates: { proxy: "entradas", inversa: true }, // rival que entra mucho → me deja rematar menos
  rematesPuerta: { proxy: "entradas", inversa: true },
  entradas: { proxy: "remates", inversa: false }, // rival que remata mucho → tengo que entrar más
  faltasCometidas: { proxy: "faltasRecibidas", inversa: false }, // rival al que le pitan muchas faltas a favor → yo cometo más
  faltasRecibidas: { proxy: "faltasCometidas", inversa: false }, // rival que comete muchas faltas → a mí me pitan más a favor
  paradas: { proxy: "rematesPuerta", inversa: false }, // rival que remata mucho a puerta → el portero para más
};
...
// Probabilidad/expectativa ajustada por contexto: parte de la media real del
// jugador y la sube o baja según cómo sea el equipo rival comparado con la
// media de la liga en la estadística relacionada.
